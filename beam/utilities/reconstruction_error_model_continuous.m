function [temporal_recon, alphas] = reconstruction_err_model_continuous(err_modes, err_eigs, state_init, B, C, dt, err_history, u_history, accel_history, state_mean, t_vec, nondim_factor, subset)
  ...
  disp('spDMD amplitudes: '); disp(alphas);

  %alphas = [0.000773188403850 + 0.025817573568824i; 0.000773188403850 - 0.025817573568824i];
  alphas = pinv(err_modes(:,subset))*err_history(:,1); % compute amplitude by projecting onto first snapshot
  disp('projection amplitudes: '); disp(alphas);
  ...
  for k = 1:max_t
    ...
    temporal_recon_k = temporal_recon_k.*nondim_factor + B*u_history(:,Nt+1);
