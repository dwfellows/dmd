...
% Read in data
data_choice = '005'; data_choice = '08';
M = str2num( strcat([ data_choice(1), '.', data_choice(2:end) ]) );
pressure_fname = strcat(['./master_data/m', data_choice, '/surface_pressure_history_m', data_choice, '.csv']);
x_fname = strcat(['./master_data/m', data_choice, '/surface_xloc_history_m', data_choice, '.csv']);
y_fname = strcat(['./master_data/m', data_choice, '/surface_yloc_history_m', data_choice, '.csv']);
xdot_fname = strcat(['./master_data/m', data_choice, '/surface_xdot_history_m', data_choice, '.csv']);
ydot_fname = strcat(['./master_data/m', data_choice, '/surface_ydot_history_m', data_choice, '.csv']);
...
if (show_data == 1)
  amp = 0.00336757;
  x = linspace(0, 0.3, 61); L_panel = 0.3;
  f_data = figure; f_data.Position = [517 527 723 420];
  v_fname = strcat(['./animations/data/m', num2str(data_choice), '_data.mp4']);
  v_data = VideoWriter(v_fname, 'MPEG-4');
  v_data.FrameRate = 60; v_data.Quality = 12; open(v_data);
  p_max = 1.05*(max(max(data)) - 1) + 1; p_min = 1.05*(min(min(data(:,2:end))) - 1) + 1;
  %p_max = 1.002; p_min = 0.998;
  for j = 1:length(data)
    subplot(211);
    plot(x ./ L_panel, data(:,j), 'LineWidth', 2); grid on;
    ylim([p_min, p_max]);
    ax = gca; ax.FontSize = afs;
    ylabel('$\vec{p} (t)$', 'interpreter', 'latex', 'fontsize', fs);

    subplot(212);
    plot(x ./ L_panel, data_y(:,j), 'LineWidth', 2); grid on;
    ax = gca; ax.FontSize = afs;
    ylim([-amp, amp]);
    xlabel('$x / L$', 'interpreter', 'latex', 'fontsize', fs);
    ylabel('$w(x,t)$', 'interpreter', 'latex', 'fontsize', fs);

    sgtitle('Flow Solution, $M = 1.06$', 'interpreter', 'latex', 'fontsize', fs);
...
%data_x = round(data_x, prec);
%data_y = round(data_y, prec);
%data_xdot = round(data_xdot, prec);
%data_ydot = round(data_ydot, prec);

% Determine how many snapshots to use
snap_level = 1;
%snapshot_begin = 3001; % temp
snapshot_begin = 1001;
snapshot_end = 5001;
...
for j = 1:length(snapshot_vec)
  ...
  w = w_mis + w_mot;
  pn = pref.*ones(1,61) + (rhoref*aref).*w;
  pn = pn ./ pref;
  lpt_pred(:,j) = pn';
...
structure = [ ones(61,1)*q(snapshot_vec); ones(61,1)*qdot(snapshot_vec) ];
accel = [ ones(61,1)*qddot(snapshot_vec) ];
...
f_singval = figure;
s_diag = diag(s);
s3001 = dlmread('./temp/svals_3001.csv'); s3001_diag = diag(s3001);
semilogy(1:length(s_diag), s_diag, 'o'); grid on; hold on;
semilogy(1:length(s3001_diag), s3001_diag, 'or');
legend('$\Sigma$, Snapshot Begin = 1001', '$\Sigma$, Snapshot Begin = 3001', 'interpreter', 'latex', 'fontsize', fs);
...
eigs_3001 = dlmread('./temp/evals_3001.csv');
...
plot(xt,yt,'--k', 'LineWidth', 2); pbaspect([1 1 1]);
...
legend('$\lambda$, Snapshot Begin = 1001', '$\lambda$, Snapshot Begin = 3001', 'Harmonic Frequency', 'interpreter', 'latex', 'fontsize', fs);
%saveas(f_eigs, eig_fname);

% Temp
f_eigs_temp=figure; f_eigs_temp.Position = [585 -848 761 617];
lh(1) = plot(eigA, 'o', 'MarkerSize', 10); grid on; hold on;
sf = sqrt( (tan(omega*pc2_dt*snapshot)^2) / (1 + tan(omega*pc2_dt*snapshot)^2) )
lh(2) = yline(sf, 'r', 'displayname', 'Structural Deformation Freq.', 'LineWidth', 2); 
lh(3) = yline(-1*sf, 'r', 'LineWidth', 2);
th = 0:0.01:2*pi; xt = cos(th); yt = sin(th);
lh(4) = plot(xt,yt,'--k', 'LineWidth', 2); pbaspect([1 1 1]);
ax = gca; ax.FontSize = 16;
xlabel('Re($\lambda$)', 'interpreter', 'latex', 'fontsize', fs);
ylabel('Im($\lambda$)', 'interpreter', 'latex', 'fontsize', fs);
legend(lh(2), 'interpreter', 'latex', 'fontsize', fs);
...
title_fname = strcat(['DMD Learned Modes, $M=', num2str(M), '$']);
...
for j = 1:4
  subplot(2,2,j);
  d_loc = d_dmd_noaccel(j,j); g = real(d_loc); omega = imag(d_loc);
  plot(x ./ L_panel, real(phi_dmd_noaccel(:,j)), 'b', 'LineWidth', 2); grid on; hold on;
  plot(x ./ L_panel, imag(phi_dmd_noaccel(:,j)), '--k', 'LineWidth', 2);
  ax = gca; ax.FontSize = afs;
  t_name = strcat(['$Re( \lambda ) = ', num2str(g), ', Im( \lambda ) = ', num2str(omega), '$']);
  title(t_name, 'interpreter', 'latex');
  if (j == 1)
    legend('Re( $\phi$ )', 'Im( $\phi$ )', 'interpreter', 'latex', 'fontsize', fs);
  end
  if (j == 1)
    ylabel('$\phi$', 'interpreter', 'latex', 'fontsize', fs);
  elseif (j==3)
    ylabel('$\phi$', 'interpreter', 'latex', 'fontsize', fs);
    xlabel('$x / L$', 'interpreter', 'latex', 'fontsize', fs);
  elseif (j==4)
...
%t_vec = (1:4001) .* (snapshot*pc2_dt); t_vec = t_vec - snapshot*pc2_dt;
t_vec = (1:(snapshot_end - snapshot_begin + 1)) .* (snapshot*pc2_dt); t_vec = t_vec - snapshot*pc2_dt;
continuous_subset = 55:56;
[continuous_temporal_recon] = reconstruction_error_model_continuous(phi_dmd_noaccel, d_dmd_noaccel, data(:,1), B_lpt, C, snapshot*pc2_dt, err, structure, accel, ones(Nx,1), t_vec);
[continuous_temporal_recon_low] = reconstruction_error_model_continuous(phi_dmd_noaccel, d_dmd_noaccel, data(:,1), B_lpt, C, snapshot*pc2_dt, err, structure, accel, ones(Nx,1), t_vec, continuous_subset);

% Compute optimal amplitudes utilize spDMD constraints
[u,s,v] = svd(err(:,1:end-1), 'econ');
r = max(size(s));
...
alphas_dmd = pinv(phi_dmd_noaccel(:,subset))*(data(:,1) - B_lpt*structure(:,1) - ones(Nx,1));
...
min_p_recon = min(min(real(continuous_temporal_recon))); min_p_recon_low = min(min(real(continuous_temporal_recon_low))); min_p = min([min_p_recon, min_p_recon_low]);
max_p_recon = max(max(real(continuous_temporal_recon))); max_p_recon_low = max(max(real(continuous_temporal_recon_low))); max_p = max([max_p_recon, max_p_recon_low]);
levels = linspace(min_p, max_p, 100);
...
ax = gca; ax.FontSize = 16;
xlabel('$x / L$', 'interpreter', 'latex', 'fontsize', fs);
title('Full CR', 'interpreter', 'latex', 'fontsize', fs);

recon_sp3 = subplot(143);
diff = real(continuous_temporal_recon_low - continuous_temporal_recon);
min_diff = min(min(diff)); max_diff = max(max(diff));
levels = linspace(min_diff, max_diff, 100);
contourf(spatial, temporal, diff, levels, 'edgecolor', 'none');
colorbar; caxis([min_diff, max_diff]); colormap('jet');
ax = gca; ax.FontSize = 16;
xlabel('$x / L$', 'interpreter', 'latex', 'fontsize', fs);
title('$\big( \vec{p}_{L}^{CR} - \vec{p}_{F} \big) / p_{in}$', 'interpreter', 'latex', 'fontsize', fs);
...
f_recon_cont_low_orig = figure; f_recon_cont_low_orig.Position = [34 138 1582 736];
spatial = linspace(0,1,Nx)';
spatial = spatial * ones(1,Nt);
temporal = ones(Nx,1) * t_phys;

min_p_orig = min(min(data)); min_p_recon_low = min(min(real(continuous_temporal_recon_low))); min_p = min([min_p_orig, min_p_recon_low]);
max_p_orig = max(max(data)); max_p_recon_low = max(max(real(continuous_temporal_recon_low))); max_p = max([max_p_orig, max_p_recon_low]);
levels = linspace(min_p, max_p, 100);
...
ax = gca; ax.FontSize = 16;
xlabel('$x / L$', 'interpreter', 'latex', 'fontsize', fs);
title('Unsteady CFD', 'interpreter', 'latex', 'fontsize', fs);
...
ax = gca; ax.FontSize = 16;
xlabel('$x / L$', 'interpreter', 'latex', 'fontsize', fs);
title('$\big( \vec{p}_{L}^{CR} - \vec{p}_{CFD} \big) / p_{in}$', 'interpreter', 'latex', 'fontsize', fs);
...
sgtitle(strcat(['Low-Order Continuous Reconstructions of Pressure vs. CFD Computation, $M = ', num2str(M), '$, SnapLev = ', num2str(snap_level)]), 'interpreter', 'latex', 'fontsize', fs);


% Temp
f_recon_cont_low_orig_temp = figure; f_recon_cont_low_orig_temp.Position = [34 138 1582 736];
spatial = linspace(0,1,Nx)';
spatial = spatial * ones(1,Nt);
temporal = ones(Nx,1) * t_phys;
...
recon_sp4 = subplot(133);
min_lpt = min(min(lpt_pred)); max_lpt = max(max(lpt_pred)); levels = linspace(min_lpt, max_lpt, 100);
contourf(spatial, temporal, lpt_pred, levels, 'edgecolor', 'none');
colorbar; caxis([min_lpt, max_lpt]); colormap('jet');
ax = gca; ax.FontSize = 16;
xlabel('$x / L$', 'interpreter', 'latex', 'fontsize', fs);
title('Piston Theory', 'interpreter', 'latex', 'fontsize', fs);

sgtitle(strcat(['Pressure Comparisons, $M = ', num2str(M), '$']), 'interpreter', 'latex', 'fontsize', fs);
