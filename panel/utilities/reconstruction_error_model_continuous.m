function [temporal_recon, alphas] = reconstruction_error_model_continuous(err_modes, err_eigs, state_init, B, C, dt, err_history, u_history, accel_history, state_mean, t_vec, nondim_term, subset)
  ...
  disp('alphas projected: '); disp(alphas);
  ...
  omega = log(omega) ./ dt; disp('omega: '); disp(omega);
  err_modes_pinv = pinv(err_modes);

  %disp('Sum of modes and amplitudes: ');
  %disp(err_modes(:,1)*alphas(1) + err_modes(:,2)*alphas(2));
  ...
  for k = 1:max_t
    ...
    temporal_recon_k = err_modes * diag(exp(omega.*t)) * alphas; % Utilize projection amplitudes
