...
% Read in data
data_choice = 'origamp'; data_choice_m = '05'; 
M = str2num( strcat([ data_choice_m(1), '.', data_choice_m(2:end) ]) );
pressure_fname = strcat(['./varyamp_data/m', data_choice_m, '/', data_choice, '/surface_pressure_history_m', data_choice_m, '_', data_choice, '.csv']);
x_fname = strcat(['./varyamp_data/m', data_choice_m, '/', data_choice, '/surface_xloc_history_m', data_choice_m, '_', data_choice, '.csv']);
y_fname = strcat(['./varyamp_data/m', data_choice_m, '/', data_choice, '/surface_yloc_history_m', data_choice_m, '_', data_choice, '.csv']);
z_fname = strcat(['./varyamp_data/m', data_choice_m, '/', data_choice, '/surface_zloc_history_m', data_choice_m, '_', data_choice, '.csv']);
xdot_fname = strcat(['./varyamp_data/m', data_choice_m, '/', data_choice, '/surface_xdot_history_m', data_choice_m, '_', data_choice, '.csv']);
ydot_fname = strcat(['./varyamp_data/m', data_choice_m, '/', data_choice, '/surface_ydot_history_m', data_choice_m, '_', data_choice, '.csv']);
zdot_fname = strcat(['./varyamp_data/m', data_choice_m, '/', data_choice, '/surface_zdot_history_m', data_choice_m, '_', data_choice, '.csv']);

data = dlmread(pressure_fname); data = data';
data_x = dlmread(x_fname); data_x = data_x';
data_y = dlmread(y_fname); data_y = data_y';
data_z = dlmread(z_fname); data_z = data_z';  z_loc = data_z(:,1); % for purposes of computing barycentric coords
...
data_x = data_x(:, snapshot_vec); data_y = data_y(:, snapshot_vec); data_z = data_z(:, snapshot_vec);
data_xdot = data_xdot(:, snapshot_vec); data_ydot = data_ydot(:, snapshot_vec); data_zdot = data_zdot(:, snapshot_vec);

% Define simulation parameters
pc2_dt_nondim = 0.001;
if (strcmp(data_choice,'halfamp'))
  amp = 2.54e-5;
...
dlmwrite('~/research/bem/mode1_modes_3d/x_plot_dmd.csv', x_plot, 'delimiter', ',', 'precision', 12);
dlmwrite('~/research/bem/mode1_modes_3d/y_plot_dmd.csv', y_plot, 'delimiter', ',', 'precision', 12);
...
B_lpt = B_lpt ./ pref;
structure = [ones(Nx*Ny,1)*q; ones(Nx*Ny,1)*qdot];
...
if (show_data == 1)
  f_data = figure;
  v_fname = strcat(['./animations/dmd_error_modes/m', num2str(data_choice_m), '_data.mp4']);
  ...
  for j = 1:length(snapshot_vec)
    ...
    subplot(133);
    p_lpt_loc_scat = B_lpt*structure(:,j); p_lpt_loc = reshape(p_lpt_loc_scat, Nx, Ny); p_lpt_loc = p_lpt_loc';
    surf(x_plot_surf, y_plot_surf, ones(Ny,Nx), p_lpt_loc, 'FaceAlpha', 0.33); hold on;
    scatter(x_plot, y_plot, sz, p_lpt_loc_scat, 'filled'); grid on;
    pbaspect([1 2 1]);
    colorbar; colormap('jet'); view(2);
    ax = gca; ax.FontSize = ffs;
    title('$\vec{p}^{\prime}_{LPT}$', 'interpreter', 'latex', 'fontsize', fs);
...
sing_val_fname = strcat(['~/individual_meetings/fellows_08032022/figures/dmd_error_panel/m', data_choice_m, '_singular_values.png']);
...
lh(2) = yline(sf, 'r', 'DisplayName', '$\omega_1$'); lh(3) = yline(-1*sf, 'r'); 
...
legend(lh(2), 'interpreter', 'latex', 'fontsize', fs, 'location', 'northeast');
%title_fname = strcat(['Eigenvalues of $\bar{A}$, w/o Accel., $M=', num2str(M), '$, SnapLev = ', num2str(snap_level)]);
title_fname = strcat(['Eigenvalues of $\bar{A}$, $M=', num2str(M), '$']);
title(title_fname, 'interpreter', 'latex', 'fontsize', fs);
eig_fname = strcat(['~/individual_meetings/fellows_08032022/figures/dmd_error_panel/m', data_choice_m, '_eigs.png']);
%saveas(f_eigs, eig_fname);

%% Find leading eigenvalues
d_dmd_diag_imag = imag(d_dmd_diag);
diff = abs(sf - d_dmd_diag_imag);
[minval, minind] = min(diff); le_ind = [minind, minind+1];
disp('le found');

dlmwrite(strcat(['~/research/bem/mode1_modes_3d/m', data_choice_m, '_modes_', data_choice, '.csv']), phi_dmd(:,le_ind), 'delimiter', ',', 'precision', 12);
...
if (show_modes == 1)
  ...
  f_modes = figure; f_modes.Position = [307 -878 1131 655];
  v_fname = strcat(['./animations/dmd_error_modes/m', num2str(data_choice_m), '.mp4']);
  ...
  for j = le_ind
    ...
    title('Re( $\phi_2$ )', 'interpreter', 'latex', 'fontsize', fs);
    ...
    title('Im( $\phi_2$ )', 'interpreter', 'latex', 'fontsize', fs);
    ...
    frame_name = strcat(['./learned_modes/varyamp/m', data_choice_m, '/', data_choice, '/mode', num2str(j), '.png']);
...
C = zeros(Np, Np);
accel = [ ones(Np,1)*qddot ];

continuous_subset = le_ind(1:2);
...
[continuous_temporal_recon_low, alphas] = reconstruction_error_model_continuous(phi_dmd, d_dmd, data(:,1), B_lpt, C, snapshot*pc2_dt, err, structure, accel, ones(Np,1), t_vec, nondim_term, continuous_subset);
dlmwrite(strcat(['~/research/bem/mode1_modes_3d/m', data_choice_m, '_amps_', data_choice, '.csv']), alphas, 'delimiter', ',', 'precision', 12);
...
if (show_full_cont_recon == 1)
  ...
  v_fname = strcat(['./animations/dmd_recon_singleDMD/m', num2str(data_choice), '_full_cont_recon_spdmd.mp4']);
...
if (show_low_cont_recon == 1)
  mode_name = strcat(['./varyamp_data/m', data_choice_m, '/', data_choice, '/leading_eigenmodes.csv']);
  dlmwrite(mode_name, phi_dmd(:,continuous_subset), 'delimiter', ',', 'precision', 12);

  eval_name = strcat(['./varyamp_data/m', data_choice_m, '/', data_choice, '/leading_evals.csv']);
  dlmwrite(eval_name, d_dmd(continuous_subset, continuous_subset), 'delimiter', ',', 'precision', 12);

  f_low_cont_recon = figure; f_low_cont_recon.Position = [166 216 1346 547];
  v_fname = strcat(['./animations/dmd_recon_singleDMD/m', num2str(data_choice_m), '/', data_choice, '_low_cont_recon_spdmd_twomode.mp4']);
  v_low_cont_recon = VideoWriter(v_fname, 'MPEG-4');
  v_low_cont_recon.FrameRate = 20; v_low_cont_recon.Quality = 12;
  ...
  for j = 1:length(t_phys)
    ...
    loc_cont_recon_surf = reshape(loc_cont_recon, Nx, Ny); loc_cont_recon_surf = loc_cont_recon_surf';
    loc_data_surf = reshape(data(:,j), Nx, Ny); loc_data_surf = loc_data_surf';
    loc_z_surf = reshape(data_z(:,j), Nx, Ny); loc_z_surf = loc_z_surf';

    max_p_data = max(max(loc_data_surf)); max_p_recon = max(max(real(loc_cont_recon_surf))); max_p = max([max_p_data, max_p_recon]);
    min_p_data = min(min(loc_data_surf)); min_p_recon = min(min(real(loc_cont_recon_surf))); min_p = min([min_p_data, min_p_recon]);
    ...
    xlabel('$x / a$', 'interpreter', 'latex', 'fontsize', fs);
    ylabel('$y / a$', 'interpreter', 'latex', 'fontsize', fs);
    %title('Low-Order Recon.', 'interpreter', 'latex', 'fontsize', fs);
    title('$p_R$', 'interpreter', 'latex', 'fontsize', fs);
    ...
    xlabel('$x / a$', 'interpreter', 'latex', 'fontsize', fs);
    %title('Original Data', 'interpreter', 'latex', 'fontsize', fs);
    title('$p_{CFD}$', 'interpreter', 'latex', 'fontsize', fs);
    ...
    xlabel('$x / a$', 'interpreter', 'latex', 'fontsize', fs);
    %title('$\%$ Difference', 'interpreter', 'latex', 'fontsize', fs);
    title('$\big( p_R - p_{CFD} \big) / p_{in}$', 'interpreter', 'latex', 'fontsize', fs);
  
%    subplot(144);
%    surf(x_plot_surf, y_plot_surf, ones(Ny,Nx), loc_z_surf, 'FaceAlpha', 0.33); hold on;
%    scatter(x_plot, y_plot, sz, data_z(:,j), 'filled'); grid on;
%    pbaspect([1 2 1]);
%    colorbar; colormap('jet'); caxis([-amp, amp]); view(2);
%    ax = gca; ax.FontSize = ffs;
%    xlabel('$x / a$', 'interpreter', 'latex', 'fontsize', fs);
%    title('$w(x,y)$', 'interpreter', 'latex', 'fontsize', fs);

    sgtitle(strcat(['Low-Order Continuous Reconstruction vs. Original Data, $M = ', num2str(M), '$, SnapLev = ', num2str(snap_level), ', $t U_{\infty} / L = ', num2str(t_loc), ', t = ', num2str(j+snapshot_begin-1), '/', num2str(snapshot_end), '$']), 'interpreter', 'latex', 'fontsize', fs);
